...
export function WhySparkBots() {
    return (
        <section className="py-20 md:py-32 px-6 md:px-12 bg-background flex flex-col items-center overflow-hidden">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 md:gap-20">
                    ...
                    <div className="flex flex-col gap-8 md:gap-[80px]">
                        {features.map((feature, index) => (
                            <div key={index} className="flex flex-col md:flex-row md:items-start gap-6 md:gap-12 pb-8 md:pb-[80px] border-b border-foreground/[0.05] last:border-0 group cursor-default">
                                <div className="text-[12px] md:text-[14px] font-medium text-foreground/20 group-hover:text-foreground transition-colors font-outfit">0{index + 1}</div>
            ...
            <div className="mt-16 md:mt-32 w-full max-w-7xl flex flex-col md:flex-row items-center justify-between p-6 md:p-12 bg-black rounded-[2rem] md:rounded-[3rem] text-white">
                <div className="flex flex-col sm:flex-row items-center gap-4 mb-6 md:mb-0">
                    <div className="flex -space-x-4">
                        {[1, 2, 3, 4].map(i => (
                            <div key={i} className="w-8 h-8 md:w-12 md:h-12 rounded-full border-2 border-black bg-slate-800 overflow-hidden relative">
                                <NextImage src={`https://i.pravatar.cc/100?u=${i}`} alt="" fill />
                            </div>
                        ))}
                    </div>
                    <p className="text-[10px] md:text-xs font-black uppercase tracking-widest text-white/60 text-center sm:text-left">Join 2,000+ parents</p>
                </div>
                <div className="flex flex-col items-center md:items-end text-center md:text-right">
                    <p className="text-xs md:text-sm italic font-medium text-white/80 mb-2">"The only kit my son didn't abandon after two days."</p>
                    <p className="text-[10px] md:text-xs font-black uppercase tracking-widest text-spark-lime">— Priya S., Mother</p>
                </div>
            </div>
        </section>
    );
